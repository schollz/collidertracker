...
func ModifyFileMetadataValue(m *model.Model, delta float32) {
	...
	case types.FileMetadataRowPlaythrough: // Playthrough (0=Sliced, 1=Oneshot, 2=Sliced + Bounce)
		modifier := createIntModifier(
			...
			0, 2, fmt.Sprintf("file metadata Playthrough for %s", m.MetadataEditingFile),
		)
		modifyValueWithBounds(modifier, delta)
